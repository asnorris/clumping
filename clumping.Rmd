...
quantile(sim$clumps, c(0.025, 0.975))

# calculate the p value.  Use pnorm to find the p value.  I used the mean function to find the mean. The p value is below 0.05 which means that the result is significantly significant and therefore we REJECT THE NULL

num_greater_than_2 <- sim %>%
  filter(clumps >= 2) %>%
  count()

num_greater_than_2/nrow(sim)

# This means that females DO CLUMP - the low p value 
...
How could we change this code to figure out if women clump more than men?

```{r}
# model to evaluate: "Causal impact of giving a break on midterm complaints. Seems to encourage a certain improvement in those students' grades."

n <- 1000
true_ability <- rnorm(n, 10, 5)
noise_1 <- rnorm(n, 0, 5)
noise_2 <- rnorm(n, 0, 5)
